...
__constant__ float deviceKernel1[4][1][7][7];

__constant__ float deviceKernel2[16][4][7][7];
...
#define mask_4d(i3, i2, i1, i0) mask[(i3) * (Channel * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    if (Map_out == 4 && Channel == 1 && K == 7) {
        if (h < Height_out && w < Width_out) {
            float acc = 0.;
            for (int c = 0; c < Channel; c++) {
                for (int p = 0; p < K; p++) {
                    for (int q = 0; q < K; q++) {
                        acc += in_4d(b, c, h+p, w+q) * deviceKernel1[m][c][p][q];
                    }
                }
            }
            out_4d(b, m, h, w) = acc;
    ...
    if (Map_out == 16 && Channel == 4 && K == 7) {
        if (h < Height_out && w < Width_out) {
            ...
            for (int c = 0; c < Channel; c++) {
                for (int p = 0; p < K; p++) {
                    for (int q = 0; q < K; q++) {
                        acc += in_4d(b, c, h+p, w+q) * deviceKernel2[m][c][p][q];
                    }
                }
            }
            out_4d(b, m, h, w) = acc;
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    if (Map_out == 4 && Channel == 1 && K == 7) {
        cudaMemcpyToSymbol(deviceKernel1, host_mask, 196 * sizeof(float));
        cudaMemcpyToSymbol(deviceKernel1, host_mask, 196 * sizeof(float));
    }

    if (Map_out == 16 && Channel == 4 && K == 7) {
        cudaMemcpyToSymbol(deviceKernel2, host_mask, 3136 * sizeof(float));
        cudaMemcpyToSymbol(deviceKernel2, host_mask, 3136 * sizeof(float));
    }
...
__host__ void GPUInterface::conv_forward_gpu_epilog(float *host_output, float *device_output, float *device_input, float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    cudaMemcpy(host_output, device_output, Batch * Map_out * Width_out * Height_out * sizeof(float), cudaMemcpyDeviceToHost);
