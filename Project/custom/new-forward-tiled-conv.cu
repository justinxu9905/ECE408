...
#define mask_4d(i3, i2, i1, i0) mask[(i3) * (Channel * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    int h_lim = min(blockIdx.z / Width_grid * TILE_WIDTH + TILE_WIDTH, Height_out);
    int w_lim = min(blockIdx.z % Width_grid * TILE_WIDTH + TILE_WIDTH, Width_out);

    if (h < Height_out && w < Width_out) {
        float acc = 0.;
        for (int c = 0; c < Channel; c++) {
            tile[ty][tx] = in_4d(b, c, h + K / 2, w + K / 2); // the most used part
            __syncthreads();
            for (int p = 0; p < K; p++) {
                if (h + p >= Height) { break; }
                for (int q = 0; q < K; q++) {
                    ...
                    int tile_y = ty + p - K / 2;
                    int tile_x = tx + q - K / 2;
                    if (tile_y >= 0 && h + p - K / 2 < h_lim && tile_x >= 0 && w + q - K / 2 < w_lim) {
                        acc += tile[tile_y][tile_x] * mask_4d(m, c, p, q);
                    } else {
